...
- (void)OKClick{
    ...
    NSLog(@"%@+%@",self.titleText,self.bodyText);
}
//重写父类方法,实现输入标题时候的编辑模式不可选,并且设置占位符的颜色和大小
...
- (void)showPromptForImageWithID:(NSString *)imageId
{
    ...
    if (!progress.cancelled){
        UIAlertAction *stopAction = [UIAlertAction actionWithTitle:@"停止下载"
        ...
        UIAlertAction *retryAction = [UIAlertAction actionWithTitle:@"重新下载"
